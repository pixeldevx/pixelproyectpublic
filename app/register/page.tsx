    ...
    <main className="min-h-[100dvh] bg-[#faf9f5] text-slate-900 lg:grid lg:grid-cols-[0.9fr_1.1fr]">
      <aside className="relative overflow-hidden bg-[#253d31] px-6 py-8 text-white sm:px-10 lg:flex lg:min-h-screen lg:flex-col lg:px-14 lg:py-12">
        ...
        <div className="relative mx-auto mt-10 max-w-lg lg:my-auto lg:py-20">
          <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-xs font-medium text-[#e4ecda]"><span className="h-1.5 w-1.5 rounded-full bg-lime-300" /> Tu próxima forma de trabajar</span>
          ...
          <p className="mt-5 max-w-sm text-sm leading-7 text-slate-300 sm:text-base">Conecta a tu equipo, organiza el trabajo y sigue cada avance desde una sola herramienta.</p>
        ...
        <div className="w-full max-w-md">
          ...
          {sent ? (
            <div>
              ...
              <p className="mt-4 text-sm leading-7 text-slate-600">Revisa <strong className="break-all font-semibold text-slate-900">{email.trim()}</strong> y abre el enlace de confirmación. Después prepararemos <strong className="font-semibold text-slate-900">{workspaceName.trim()}</strong> para que empieces tu prueba.</p>
            ...
            <div>
              <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.17em] text-[#c4472b]"><CheckCircle2 size={16} /> Prueba gratuita de 14 días</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-tight text-[#20352f] sm:text-4xl">Tu equipo empieza aquí.</h2>
              <p className="mt-3 text-sm leading-6 text-slate-500">Crea tu cuenta y tu organización. Sin tarjeta de crédito.</p>
              <form className="mt-8 space-y-5" onSubmit={handleRegister}>
                ...
                <button type="submit" disabled={submitting || loading} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#c94d32] px-5 text-sm font-semibold text-white shadow-lg shadow-[#dc5638]/15 transition hover:bg-[#bd432a] disabled:cursor-wait disabled:opacity-60">{submitting || loading ? <><Loader2 size={18} className="animate-spin" /> {submitting ? 'Creando tu cuenta…' : 'Verificando sesión…'}</> : <>Crear mi espacio gratis <ArrowRight size={17} /></>}</button>
                <p className="flex items-center justify-center gap-2 text-xs text-slate-500"><Check size={14} className="text-emerald-600" /> Confirma tu correo para comenzar. Sin cobros automáticos.</p>
              </form>
