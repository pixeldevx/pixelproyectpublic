...
import { getWorkspaceServerClient, workspaceErrorStatus, WorkspaceAccessError } from '@/lib/workspaces/server';
...
const getRedirectTo = (request: NextRequest) => {
  ...
  if (!configuredUrl) throw new WorkspaceAccessError('Configura el dominio de Pixel para generar invitaciones.', 503);
  let origin: URL;
  try { origin = new URL(configuredUrl.startsWith('http') ? configuredUrl : `https://${configuredUrl}`); }
  catch { throw new WorkspaceAccessError('El dominio configurado no es válido.', 503); }
  if (origin.protocol !== 'https:' && !(process.env.NODE_ENV === 'development' && ['localhost','127.0.0.1'].includes(origin.hostname))) {
    throw new WorkspaceAccessError('El dominio de Pixel debe usar HTTPS.', 503);
  }
...
export async function handleWorkspaceInvite(request: NextRequest, supabase: any) {
  try {
