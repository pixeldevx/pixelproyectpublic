        ...
        <div className="space-y-5 p-6">
          {result ? (
            ...
            {canSave ? <>
              ...
              {target.kind === 'workspace' && <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium">Estado<select value={status} disabled={protectedWorkspace} onChange={(event) => setStatus(event.target.value as typeof status)} className={`${fieldClass} mt-2`}><option value="trial">Prueba gratuita</option><option value="active">Activo</option><option value="suspended" disabled={protectedWorkspace}>Suspendido</option></select>{protectedWorkspace && <span className="mt-1 block text-xs font-normal text-slate-500">El espacio del administrador global conserva su acceso.</span>}</label><label className="block text-sm font-medium">Fin de la prueba<input type="date" disabled={protectedWorkspace} required={status === 'trial'} value={trialEnd} onChange={(event) => setTrialEnd(event.target.value)} className={`${fieldClass} mt-2`} /><span className="mt-1 block text-xs font-normal text-slate-500">Hasta las 23:59, hora de Colombia.</span></label></div>}
